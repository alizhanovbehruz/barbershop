...
from keyboards.default.selskey import back, contback
...
async def selss(message :types.CallbackQuery):
    await message.message.answer("Введите полное И.Ф:", reply_markup=back)
...
async def cont(mess: types.ContentType, state = FSMContext):
    ...
    async with state.proxy() as data:
        data["phone"]=numb_p
    await mess.answer("Когда хотите преобрести стрижку (Введите в формате=dd/mm/yyyy)",reply_markup=back)
    await user_sels.next()
